...
export default function Sidebar({ mobileOpen, onClose }: { mobileOpen: boolean; onClose: () => void }) {
  ...
  return (
    <>
      {mobileOpen && <button aria-label="Close sidebar" className="fixed inset-0 z-40 bg-slate-900/45 md:hidden" onClick={onClose} />}
      <aside className={`fixed md:static inset-y-0 left-0 z-50 md:z-auto w-60 bg-slate-900 min-h-screen flex flex-col flex-shrink-0 transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
      <div className="px-5 py-6 border-b border-slate-800">
        <div className="flex items-center gap-2.5">
          ...
          <button onClick={onClose} className="ml-auto md:hidden w-11 h-11 rounded-xl border border-slate-700 text-slate-200 hover:bg-slate-800 flex items-center justify-center" aria-label="Close sidebar">
            <X size={18} />
          </button>
        </div>
      </div>
